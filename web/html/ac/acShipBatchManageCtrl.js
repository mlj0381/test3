function changeTabCallback(){
	var appElement = document.querySelector('[ng-controller=acShipBatchManageCtrl]');
	var scope=angular.element(appElement).scope();
	scope.doQuery();
	scope.$apply();
}
var proveManageApp = angular.module("acShipBatchManageApp", ['commonApp']);
proveManageApp.controller("acShipBatchManageCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) {
	var proveManage={
		init:function(){
			$scope.query = {};
			this.bindEvent();
		...
		doQuery:function(){
			$timeout(function(){
					...
					$scope.query.page=1;
					var url = "acProveManageBO.ajax?cmd=queryAcDtlShipBatchNew";
		...
		//核销请求
		 toPostTrue : function(postData){
		    $scope.dealTrue = true;
			$("#hexiao").html("核销中。。");
			var param = {"postData":postData,"type":4};
