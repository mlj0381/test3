...
loginApp.controller("loginCtrl", ["$scope","commonService","$rootScope",function($scope,commonService,$rootScope) {
	...
	var login={
			...
			queryCompanyInfo:function(){
				var that=this;
				commonService.postUrl("portalBusiBO.ajax?cmd=queryCompanyInfoChain",{tenantCode:$scope.tenantCode},function(data){
			...
			submit:function(version){
				...
				var url = "portalBusiBO.ajax?cmd=doLoginChain";
				commonService.postUrl(url,login.selectId,function(data){
					...
					if(data != null && data != undefined && data != "" && data.result=="1"){
						...
						window.location = "/toMain.html?ver="+version;
