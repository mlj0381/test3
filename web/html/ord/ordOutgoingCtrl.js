...
var operType = getQueryString("operType");// 操作类型(1:详情; 2:修改)
var ordOutgoingApp = angular.module("ordOutgoingApp", ['commonApp']);

ordOutgoingApp.controller("ordOutgoingCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) {
	var bill = {
		init:function(){
			this.bindEvent();
			this.initData();
		...
		initData:function(){
			...
			$scope.roateData = {};
			$scope.transitOutgoing = {};// 中转外发信息
			$scope.cmCarrCompanies = {};
		},
		paymentChange:function(){
			if($scope.transitOutgoing.paymentType == 1){// 现付
				$scope.transitOutgoing.cashPayment = $scope.transitOutgoing.outgoingFee; // 现付
				$scope.transitOutgoing.freightCollectPayment=0;// 提付
				$scope.transitOutgoing.receiptPayment=0;// 回单付
				$scope.transitOutgoing.monthlyPaymeny=0;// 月结
			}else if($scope.transitOutgoing.paymentType == 2){// 到付
				$scope.transitOutgoing.cashPayment = 0; // 现付
				$scope.transitOutgoing.freightCollectPayment=$scope.transitOutgoing.outgoingFee;// 提付
				$scope.transitOutgoing.receiptPayment=0;// 回单付
				$scope.transitOutgoing.monthlyPaymeny=0;// 月结
			}else if($scope.transitOutgoing.paymentType == 3){// 月结
				$scope.transitOutgoing.cashPayment = 0; // 现付
				$scope.transitOutgoing.freightCollectPayment=0;// 提付
				$scope.transitOutgoing.receiptPayment=0;// 回单付
				$scope.transitOutgoing.monthlyPaymeny=$scope.transitOutgoing.outgoingFee;// 月结
			}else if($scope.transitOutgoing.paymentType == 4){// 回单付
				$scope.transitOutgoing.cashPayment = 0; // 现付
				$scope.transitOutgoing.freightCollectPayment=0;// 提付
				$scope.transitOutgoing.receiptPayment=$scope.transitOutgoing.outgoingFee;// 回单付
				$scope.transitOutgoing.monthlyPaymeny=0;// 月结
			} else {
				$scope.transitOutgoing.cashPayment = 0; // 现付
				$scope.transitOutgoing.freightCollectPayment=0;// 提付
		...
		registerKeyEvent: function(){
			var returnKeyEventDomEleIds = ["_carrierCompanyId", "_outgoingTrackingNum", "_linkerName", "_linkerPhone", "_deliveryPhone", 
			                               "_deliveryAddress", "_outgoingFee", "_transitOutgoingRemarks", "_transitOutgoingPaymentType",  
			                               "_transitOutgoingCashPayment", "_transitOutgoingFreightCollectPayment", "_transitOutgoingReceiptPayment",
			                               "_transitOutgoingMonthlyPaymeny"];
		...
		initOrderView:function(orderId){
			...
			commonService.postUrl("orderInfoBO.ajax?cmd=queryOrderInfoDetailForTransitOutgoing",{"orderId":orderId},function(data){
				...
				if(data != null && data != undefined && data != ""){
					...
					//查询承运商
					$scope.changeCarrierCompany(data.items[0].orderInfo.orderId);
					// 这里的中转网点要根据订单的开单网点来
		...
		initTransitOutgoing: function(orderId) {
			commonService.postUrl("orderInfoBO.ajax?cmd=queryOrderRelTransitOutgoing",{"orderId":orderId},function(data){
				if (undefined != data && typeof data == 'object') {
					$scope.transitOutgoing = data;
					$scope.transitOutgoing.paymentType = $scope.transitOutgoing.paymentType + "";
					$scope.transitOutgoing.outgoingFee = $scope.transitOutgoing.outgoingFee / 100; 
					$scope.transitOutgoing.cashPayment = $scope.transitOutgoing.cashPayment / 100;
					$scope.transitOutgoing.freightCollectPayment = $scope.transitOutgoing.freightCollectPayment / 100;
					$scope.transitOutgoing.receiptPayment = $scope.transitOutgoing.receiptPayment / 100; 
					$scope.transitOutgoing.monthlyPaymeny = $scope.transitOutgoing.monthlyPaymeny / 100;
				}
			});
		},
		//－修改和显示－－处理费用金额
...
//				var value = eval("$scope."+valueName).replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1');
		...
		submit: function(){
			...
			if(!$scope.checkStrIsBlank($scope.transitOutgoing.outgoingFee)){
				commonService.alert("请填写外发费", function(){
					jQuery("#_outgoingFee").focus();
				});
				return false;
			}
			if(!$scope.checkStrIsBlank($scope.transitOutgoing.paymentType) || $scope.transitOutgoing.paymentType <= 0){
				commonService.alert("请选择付款方式", function(){
					jQuery("#_transitOutgoingPaymentType").focus();
				});
				return false;
			}
			
			if($scope.transitOutgoing.paymentType == 1 && $scope.transitOutgoing.cashPayment <=0){
				commonService.alert("请输入填写现付金额", function(){
					jQuery("#_transitOutgoingCashPayment").focus();
				});
				return false;
			}else if($scope.transitOutgoing.paymentType == 2 && $scope.transitOutgoing.freightCollectPayment <=0){
				commonService.alert("请填写提付金额", function(){
					jQuery("#_transitOutgoingFreightCollectPayment").focus();
				});
				return false;
			}else if($scope.transitOutgoing.paymentType == 3 && $scope.transitOutgoing.monthlyPaymeny <=0){
				commonService.alert("请填写月结金额",function(){
					jQuery("#_transitOutgoingmonthlyPaymeny").focus();
				});
				return false;
			}else if($scope.transitOutgoing.paymentType == 4 && $scope.transitOutgoing.receiptPayment <=0){
				commonService.alert("请填写回单付金额", function(){
					jQuery("#_transitOutgoingReceiptPayment").focus();
				});
				return;
			}else if($scope.transitOutgoing.paymentType == 5){// 
				if($scope.transitOutgoing.cashPayment <=0 && $scope.transitOutgoing.freightCollectPayment <=0 
						&& $scope.transitOutgoing.monthlyPaymeny<=0 && $scope.transitOutgoing.receiptPayment){
					commonService.alert("请选择输入付款金额", function(){
						jQuery("#_transitOutgoingCashPayment").focus();
					});
					return false;
				}
				
				var payNumber = parseFloat($scope.transitOutgoing.cashPayment) + parseFloat($scope.transitOutgoing.freightCollectPayment) 
							  + parseFloat($scope.transitOutgoing.monthlyPaymeny) + parseFloat($scope.transitOutgoing.receiptPayment);
				if(payNumber != $scope.transitOutgoing.outgoingFee){ 
					commonService.alert("付款金额必须等于外发费", function(){
						jQuery("#_transitOutgoingCashPayment").focus();
					});
					return false;
				}
