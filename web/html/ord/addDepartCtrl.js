var addDepartApp = angular.module("addDepartApp", ['commonApp']);
addDepartApp.controller("addDepartCtrl", ["$scope","commonService","$timeout","$filter",function($scope,commonService,$timeout,$filter) {
	$scope.payDot=[];//配送网点
	$scope.query={};//查询条件
	...
	var addDepart={
		...
		onRigthInputChange:function(row){
			...
			$scope.selectMatchList=$filter("filter")(addDepart.selectMatchListTmp,filtObj);
//			$scope.filtSelectdData();
		...
		doQuery:function(){
			...
			commonService.postUrl(url,$scope.query,function(data){
				...
				if(batchNum==null || batchNum==undefined || batchNum==""){
					if(!addDepart.isQuery){
						$scope.selectMatchList=[];
						addDepart.selectMatchListTmp=[];
					}
				}
		...
		leftBtselect:function(data) {
			//判断是否重复
			if(data!=undefined && addDepart.selectMatchListTmp!=undefined && addDepart.selectMatchListTmp.length>0){
				for(var i in addDepart.selectMatchListTmp){
					var vl=addDepart.selectMatchListTmp[i];
			...
			//添加到右边的表格
			addDepart.selectMatchListTmp.push(data);
			$scope.selectMatchList=addDepart.selectMatchListTmp;
		...
		onLeftBtClick:function(f,d){
			if(f=="all"){
				...
				if($scope.matchList!=null && $scope.matchList!=undefined){
					for(var j=0;j<$scope.matchList.length;j++){
						addDepart.selectMatchListTmp.push($scope.matchList[j]);
					}
					$scope.selectMatchList=addDepart.selectMatchListTmp;
					$scope.matchList=[];
				}
		...
		onRigthBtClick:function(f,d){
			if(f=="all"){
				...
				if($scope.selectMatchList!=null && $scope.selectMatchList!=undefined){
					...
					if(addDepart.selectMatchListTmp!=undefined && addDepart.selectMatchListTmp!=null){
						if(addDepart.selectMatchListTmp.length==$scope.selectMatchList.length){
							//如果右边的数据是在没有过滤的情况下选择了全部，就直接全部过去，不需要处理
							$scope.selectMatchList=[];
							addDepart.selectMatchListTmp=[];
						}else if(addDepart.selectMatchListTmp.length>$scope.selectMatchList.length){
							//如果右边的数据是在过滤的情况下，点击全部，只是把过滤后的数据进行移除
							for(var j=0;j< $scope.selectMatchList.length;j++){
								var trackingNum=$scope.selectMatchList[j]["trackingNum"];
								for(var i=0;i<addDepart.selectMatchListTmp.length;i++){
									var trackingNumTemp=addDepart.selectMatchListTmp[i]["trackingNum"];
									if(trackingNumTemp==trackingNum){
										addDepart.selectMatchListTmp.splice(i,1);
									}
								}
							}
		...
		statisticsMatchSel:function(){
			...
			if(addDepart.selectMatchListTmp!=null && addDepart.selectMatchListTmp.length>0){
				$scope.matchCountSel=addDepart.selectMatchListTmp.length;
				for(var i=0;i<addDepart.selectMatchListTmp.length;i++){
					if(addDepart.selectMatchListTmp != null && addDepart.selectMatchListTmp != undefined 
							&& addDepart.selectMatchListTmp[i]!=null && addDepart.selectMatchListTmp[i]!=undefined ){
						$scope.freightCollect+=(parseInt(addDepart.selectMatchListTmp[i].freightCollect)/100);
						$scope.collectingMoney+=(parseInt(addDepart.selectMatchListTmp[i].collectingMoney)/100);
						console.log($scope.collectingMoney);
						$scope.actualVolume+=addDepart.selectMatchListTmp[i].volume;
						$scope.actualWeight+=addDepart.selectMatchListTmp[i].weight;
					}
				}
			}
		},
		...
		rigthBtselect:function(data) {
			...
			for(var j=0;j<addDepart.selectMatchListTmp.length;j++){
				if(addDepart.selectMatchListTmp[j]!=null && addDepart.selectMatchListTmp[j]!=undefined 
						&& data.trackingNum==addDepart.selectMatchListTmp[j].trackingNum){
					addDepart.selectMatchListTmp.splice(j,1);
				}
			}
		...
		next:function(n){
    		if(n==1){
    			if(addDepart.selectMatchListTmp.length<=0){
    			...
    			for(var i=0;i<addDepart.selectMatchListTmp.length;i++){
    				var bool=false;
    				var destCity = addDepart.selectMatchListTmp[i].distributionOrgName;
    				addDepart.selectMatchListTmp[i].goodsNames=addDepart.selectMatchListTmp[i].products;
		...
		clearSelect:function(){
			...
			if($scope.selected.length>0){
				...
				for(var i=0;i<$scope.selected.length;i++){
					...
					for(var j=0;j<addDepart.selectMatchListTmp.length;j++){
						if(addDepart.selectMatchListTmp[j]!=null && addDepart.selectMatchListTmp[j]!=undefined 
								&& trackingNumTemp==addDepart.selectMatchListTmp[j].trackingNum){
							addDepart.selectMatchListTmp.splice(j,1);
						}
					}
				}
		...
		doSave:function(){
	        ...
	        for(var i=0;i<addDepart.selectMatchListTmp.length;i++){
	        	if(i==0){
	        		orderId=addDepart.selectMatchListTmp[i].orderId;
	        	}else{
	        		orderId=orderId+","+addDepart.selectMatchListTmp[i].orderId;
	        	}
	        }
			$scope.form.orderId=orderId;
			var tips="所选车辆["+$scope.form.plateNumber+"]，可配置重量、体积为["+$scope.form.weight+"吨,"+$scope.form.volume
						+"立方]，当前配置信息中，货物重量、体积为["+($scope.actualWeight/1000).toFixed(2)+"吨,"+$scope.actualVolume.toFixed(2)+"立方]，";
			...
			commonService.confirm(tips,function(){
				...
				commonService.postUrl(url,param,function(data){
					...
					if(data!=null && data!=undefined && data!=""){
						...
						$scope.selectMatchList=[];
						addDepart.selectMatchListTmp=[];
//						$scope.diy.loadData(addDepart.selectMatchListTmp);
						
						$scope.form.transportContract ="";
						commonService.confirm("操作成功，是否继续配载？",function(){
							addDepart.isQuery=false;
							$scope.doQuery();
							$scope.next(2);
						},function(){
							commonService.closeToParentTab(false);
    		...
    		if(batchNum!=null && batchNum!=undefined && batchNum!=''){
    			...
    			commonService.postUrl(url,queryString,function(data){
    				...
    				for (var i = 0; i < data.list.length; i++) {
    					...
    					addDepart.selectMatchListTmp.push(data.list[i]);
