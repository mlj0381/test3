var transitAccountApp = angular.module("transitAccountApp", ['commonApp']);
transitAccountApp.controller("transitAccountCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) {
	var transitAccount={
		init:function(){
			$scope.isShow = false;
			this.userData();
		...
		doQuery:function(){
			...
			transitAccount.query.page=1;
			transitAccount.query.transitOutgoingState=1;
			$timeout(function(){
				$scope.p.load({
								url:url,
								params:transitAccount.query,
