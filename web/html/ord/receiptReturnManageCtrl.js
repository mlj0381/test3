function changeTabCallback(){
	var appElement = document.querySelector('[ng-controller=receiptReturnManageCtrl]');
...
var receiptReturnManageApp = angular.module("receiptReturnManageApp", ['commonApp']);
receiptReturnManageApp.controller("receiptReturnManageCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) {
	var receiptManage={
			init:function(){
				this.userData();
				this.doQuery();
			...
			printTable: function(){
				...
				ps.receiptType=2;
				var num =[12,13];//定义一个数组     12、已寄出（未返回） 13、已返回
				rs.repeictNumber=num;
				ps._ALLEXPORT = 1;
			...
			doQuery:function(){
				...
				var url = "orderInfoBO.ajax?cmd=doReceiptQuery";
				receiptManage.params.page=1;
				receiptManage.params.receiptType=2;
				var num =[12,13];//定义一个数组     12、已寄出（未返回） 13、已返回
			...
			receiptReturn:function(){
				...
				if($scope.receiptStates!=12){
					commonService.alert("不是回单已寄出的数据不能进行回单返回!");
					return;
				}
				var tips="确认是否返回";
				/*var sureText="";
				...
				commonService.confirm(tips,function(){
					var param = {"orderId":orderId,"isPay":1,"receiptStat":13};
			...
			delReturn:function(){
				...
				if($("input[name='check-1']:checked").length>1){
				   ...
				   var num = new Array();
					...
					commonService.confirm("确认取消回单返回",function(){
					    var param={"orderId":num,"receiptState":12};
