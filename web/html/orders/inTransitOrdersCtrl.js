var inTransitOrdersApp = angular.module("inTransitOrdersApp", ['commonApp','tableCommon']);
inTransitOrdersApp.controller("inTransitOrdersCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) {
	var inTransitOrders={
		init:function(){
			this.bindEvent();		
		},
		...
		bindEvent:function(){
			$scope.head=inTransitOrders.head;
		    ...
		    $timeout(function(){
				$scope.table.mouseleave = inTransitOrders.mouseleave;
				$scope.table.enterCallback = inTransitOrders.enterCallback;
			},200);
		    $scope.gxArriveGoods=this.gxArriveGoods;
		},
		gxArriveGoods:function(){
			...
			var url="ordOrdersWebBO.ajax?cmd=gxArriveGoods";
