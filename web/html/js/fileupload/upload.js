...
app.controller('MyCtrl', [ '$scope', '$http', '$timeout', '$compile', '$upload', function($scope, $http, $timeout, $compile, $upload) {
	...
	$scope.uploadPic = function(files1,files2,busiType) {
		$scope.formUpload = true;
        $scope.code = ""+parseInt(Math.random()*9999999999+1);
        if(!!busiType){
            $scope.busiType = busiType;
        }
		if (files1 != null) {
			generateThumbAndUpload(files1[0])
		}
        if (files2 != null) {
            generateThumbAndUpload(files2[0])
        }
	};
	...
	function uploadUsing$upload(file) {
		file.upload = $upload.upload({
			url: 'http://localhost:8080/admin/upload' + $scope.getReqParams(),
			method: 'POST',
			headers: {
				'my-header' : 'my-header-value'
			},
			fields: {username: $scope.username,code: $scope.code,busiType:$scope.busiType,userType:$scope.userType},
			file: file,
