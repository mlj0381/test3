...
function displayTable(){
	 ...
	 try {
		 ...
		 document.getElementsByClassName("table_height")[0].style.height=parseInt(height)-ss-200;
...
/**
 * ng-Enter-Function="doQuery()"
 * enter回车触发方法
 */
commonApp.directive("ngEnterFunction",['commonService','$document',function(commonService,$document){
	return {
	    restrict:"A",
	    link:function(scope, element, attrs){
	    	 element.bind("keydown keypress", function (event) {
	             if(event.which === 13) {
	                 scope.$apply(function (){
	                     scope.$eval(attrs.ngEnterFunction);
	                 });
	...
	myFileUp.compile=function(element, attrs){
		return function($scope, element, attrs){
			...
			var html=
				...
				'				<a class="tx" href="javascript:void(0);" ng-show="'+objUpdate+'" ng-click="'+objName+'.clike()">修改图片</a>'+
 ...
 * 
 */
commonApp.directive("myDatePicker",['$parse',function($parse){
                    return {
                        restrict:"A",
                        link:function(scope, element, attr){
                        	var obj={};
		...
		return function( $scope, element, attrs ) {
			var page={
	        		...
	        		downloadExcelFile:function(){
	        			if(this.params.params._sum!=undefined&&this.params.params._sum!=null){
	        			...
	        			commonService.downloadExcelFile(queryUrl,this.params.params,excelLables,excelKeys,"",page.tableName,"1");
...
//					+' </tr>'
					...
					+	'<span ng-if="t.number!=undefined && t.isshow0 " style="width:{{t.width == undefined ? 140 : t.width}}px;white-space: nowrap !important;word-break: keep-all;overflow: hidden;text-overflow: ellipsis;display: block;text-align:center;padding: 0 5px;">{{(t.isEdit)}}{{$eval("d."+t.code) == 0 ? "0" : $eval("d."+t.code) | number: t.number}}</span>'
					+	'<span ng-if="t.number!=undefined && (t.isshow0 ==undefined || !t.isshow0) " style="width:{{t.width == undefined ? 140 : t.width}}px;white-space: nowrap !important;word-break: keep-all;overflow: hidden;text-overflow: ellipsis;display: block;text-align:center;padding: 0 5px;">{{(t.isEdit)}}{{$eval("d."+t.code) | filterString | number: t.number}}</span>'
					+	'<span ng-if="t.number==undefined" style="width:{{t.width == undefined ? 140 : t.width}}px;white-space: nowrap !important;word-break: keep-all;overflow: hidden;text-overflow: ellipsis;display: block;text-align:center;padding: 0 5px;" title="{{$eval(\'d.\'+t.code)}}">{{(t.isEdit)}}{{$eval("d."+t.code)}}</span>'
					+'</span>'
 ...
 * 
 */
commonApp.directive("myDoubleVal", ["$parse",function ($parse) {
        return {
            require:"ngModel",
 ...
 * 
 */
commonApp.directive("myNumVal", ["$parse",function ($parse) {
        return {
            require:"ngModel",
            link:function(scope,ele,attrs,ctrl) {
            	var methodName='change';
...
commonApp.service( 'commonService', ['$http','$rootScope','$sce', '$interval',function( $http,$rootScope,$sce,$interval ) {
	 ...
	 */
	var downloadExcelFile=function(queryUrl,params,excelLables,excelKeys,filename,tableName,isAscTrue){
		if(params._sum!=undefined&&params._sum!=null){
			delete params._sum;
			}
        ...
        var tableName = tableName||queryBo+"@"+queryCmd;
        var filename = filename||"";

		var toUrl = signUrl(url+"&queryUrl="+queryBo+"|"+queryCmd+"&excelKeys="+excelKeys+"&excelLables="+excelLables+queryParamStr+"&_ALLEXPORT=1&fileName="+filename+"&tableName="+tableName);
		if(typeof isAscTrue != "undefined"){
            toUrl = signUrl(url+"&queryUrl="+queryBo+"|"+queryCmd+"&excelKeys="+excelKeys+"&excelLables="+excelLables+queryParamStr+"&_ALLEXPORT=1&fileName="+filename+"&tableName="+tableName+"&isAscTrue="+isAscTrue);
        }
        ...
        window.top.showProgressBar(1);
        var checkUrl=queryBo+".ajax?cmd=checkFinishDownLoad&time="+(new Date()).getTime()+"&tableName="+tableName;;

        that.postUrl(checkUrl,"",function(data){
            if(data.result=="true"){
                window.top.showProgressBar(100);
                var frameDownloadExcel = document.createElement("iframe");
                frameDownloadExcel.id = "frameDownloadExcel";
                frameDownloadExcel.src = encodeURI(signUrl(queryBo+".ajax?cmd=downloadExcelFileFromServer&tableName="+tableName));
                frameDownloadExcel.style.display = "none";
                document.body.appendChild(frameDownloadExcel);
                window.top.hideProgressBar();
            } else {
                that.inter=$interval(function() {
                    that.postUrl(checkUrl,"",function(data){
                        var rate = parseInt(data.rate)-5;
                        if(rate>0){
                            window.top.showProgressBar(rate);
                        }
                        if(data.result=="true"){
                            window.top.showProgressBar(100);
