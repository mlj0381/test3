...
(function () {
    var canvas = $G("J_brushBoard"),
        ...
        drawStep = [], //undo redo瀛桦偍
        drawStepIndex = 0; //undo redo鎸囬拡

    scrawl.prototype = {
        isScrawl:false, //鏄惁娑傞甫
        brushWidth:-1, //鐢荤玛绮楃粏
        brushColor:"", //鐢荤玛棰滆壊

        initOptions:function (options) {
            var me = this;
            me.originalState(options);//鍒濆椤甸溃钟舵€?
            me._buildToolbarColor(options.colorList);//锷ㄦ€佺敓鎴愰鑹查€夋嫨板嗗悎

            me._addBoardListener(options.saveNum);//娣诲姞鐢绘澘澶勭悊
            me._addOPerateListener(options.saveNum);//娣诲姞undo redo clearBoard澶勭悊
            me._addColorBarListener();//娣诲姞棰滆壊阃夋嫨澶勭悊
            me._addBrushBarListener();//娣诲姞鐢荤玛澶у皬澶勭悊
            me._addEraserBarListener();//娣诲姞姗＄毊澶у皬澶勭悊
            me._addAddImgListener();//娣诲姞澧炴坊鑳屾櫙锲剧墖澶勭悊
            me._addRemoveImgListenter();//鍒犻櫎鑳屾櫙锲剧墖澶勭悊
            me._addScalePicListenter();//娣诲姞缂╂斁澶勭悊
            me._addClearSelectionListenter();//娣诲姞娓呮阃変腑钟舵€佸鐞?

            me._originalColorSelect(options.drawBrushColor);//鍒濆鍖栭鑹查€変腑
            me._originalBrushSelect(options.drawBrushSize);//鍒濆鍖栫敾绗旈€変腑
            me._clearSelection();//娓呮阃変腑钟舵€?
        },

        originalState:function (options) {
            var me = this;

            me.brushWidth = options.drawBrushSize;//鍚屾鐢荤玛绮楃粏
            me.brushColor = options.drawBrushColor;//鍚屾鐢荤玛棰滆壊

            context.lineWidth = me.brushWidth;//鍒濆鐢荤玛澶у皬
            context.strokeStyle = me.brushColor;//鍒濆鐢荤玛棰滆壊
            context.fillStyle = "transparent";//鍒濆鐢诲竷鑳屾櫙棰滆壊
            context.lineCap = "round";//铡婚櫎阌娇
            context.fill();
        },
            ...
            domUtils.on(canvas, ["mousedown", "mousemove", "mouseup", "mouseout"], function (e) {
                ...
                switch (e.type) {
                    case 'mousedown':
                        ...
                        isMouseMove = false;
                        me.isScrawl = true;
                        startX = e.clientX - margin;//10涓哄杈硅窛镐诲拰
        ...
        getCanvasData:function () {
            ...
            if (img) {
                ...
                context.fillStyle = "#fff";//閲岖疆鐢诲竷鑳屾櫙鐧借壊
