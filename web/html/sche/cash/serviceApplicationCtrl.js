var myApp = angular.module("serviceApplicationApp", ['commonApp']);
myApp.controller("serviceApplicationCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) {
	var myManage={
		init:function(){
			$scope.query = {};
			$scope.add = {};
...
//				if (document.getElementById("checkbox").checked) {
		...
		getApplicationByIdView:function(){
			...
			commonService.openTab(batchNum.join(","),"提现明细","/sche/cash/viewApplication.html?service=1&id="+batchNum.join(","));
		},
		/**申请提现**/
		doSave:function(){
			commonService.openTab(121,"提现申请","/sche/cash/serviceToApplication.html");
		}
	};
	myManage.init();
}]);
