var busiGSTaskManageApp = angular.module("busiGSTaskManageApp", ['commonApp']);
busiGSTaskManageApp.controller("busiGSTaskManageCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) {
	var busiGSTaskManage={
		init:function(){
			this.bindEvent();
			this.doQuery();
			this.selectProvince();
		...
		selectProvince:function(){
			...
			commonService.postUrl(url,param,function(data){
				if(data!=null && data!=undefined && data!=""){
						...
						busiGSTaskManage.query.provinceId=data.items[0].id;
	 	    	}
			});
		},
		selectCity:function(provinceId){
			if(parseInt(provinceId)>0){
				...
				commonService.postUrl(url,param,function(data){
					if(data!=null && data!=undefined && data!=""){
							$scope.cityData=data;
							busiGSTaskManage.query.cityId=data.items[0].id;
		 	    	}
				});
			}
		},
			...
			if(parseInt(cityId)>0){
				...
				commonService.postUrl(url,param,function(data){
					if(data!=null && data!=undefined && data!=""){
							...
							busiGSTaskManage.query.countyId=data.items[0].id;
		 	    	}
				});
			}
		},
			...
			if(parseInt(districtId)>0){
				...
				commonService.postUrl(url,param,function(data){
					if(data!=null && data!=undefined && data!=""){
							...
							busiGSTaskManage.query.townId=data.items[0].id;
		 	    	}
				});
			}
		},
