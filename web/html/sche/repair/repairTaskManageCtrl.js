var repairTaskManageApp = angular.module("repairTaskManageApp", ['commonApp']);
repairTaskManageApp.controller("repairTaskManageCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) {
	var repairTaskManage={
			init:function(){
				this.bindEvent();
				this.doQuery();
				this.selectProvince();
			...
			shceDisSf:function(){
				commonService.confirm("运单号["+$scope.sfInfo.wayBillId+"],指派给"+$scope.sfInfo.name+"师傅，维修费："+$scope.repairFee+"元。确定分配？",function(){
					var param = {"taskId":$scope.sfInfo.taskId,"wayBillId":$scope.sfInfo.wayBillId,"repairFee":$scope.repairFee,"sfUserId":$scope.sfInfo.sfUserId,"userType":1,"scheType":1};
			...
			selectProvince:function(){
				...
				commonService.postUrl(url,param,function(data){
					if(data!=null && data!=undefined && data!=""){
							...
							repairTaskManage.query.provinceId=data.items[0].id;
		 	    	}
				});
			},
			selectCity:function(provinceId){
				if(parseInt(provinceId)>0){
					...
					commonService.postUrl(url,param,function(data){
						if(data!=null && data!=undefined && data!=""){
								$scope.cityData=data;
								repairTaskManage.query.cityId=data.items[0].id;
			 	    	}
					});
				}
			},
				...
				if(parseInt(cityId)>0){
					...
					commonService.postUrl(url,param,function(data){
						if(data!=null && data!=undefined && data!=""){
								...
								repairTaskManage.query.countyId=data.items[0].id;
			 	    	}
					});
				}
			},
				...
				if(parseInt(districtId)>0){
					...
					commonService.postUrl(url,param,function(data){
						if(data!=null && data!=undefined && data!=""){
								...
								repairTaskManage.query.townId=data.items[0].id;
			 	    	}
					});
				}
			},
			...
			moreRecommend:function(taskId,wayBillId,cityName){
					if(document.getElementById("dhl_map").style.display=='none'){
						...
						repairTaskManage.mapInit(cityName,taskId);
					}else{
							document.getElementById("dhl_map").style.display="none";
				...
				$("input[name='check-1']:checked").each(function(){
					if($(this).val()!=null&&$(this).val()!=undefined&&$(this).val()!=''){
						...
						if(sfInfo==null||sfInfo==undefined||sfInfo==''){
							...
							$scope.repairFee=data.repairFee;
						}else{
							$scope.sfInfo.taskId=data.taskId;
							$scope.sfInfo.wayBillId=data.wayBillId;
						}
