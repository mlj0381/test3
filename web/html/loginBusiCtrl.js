var loginBusiApp = angular.module("loginBusiApp", ['commonApp']);
loginBusiApp.controller("loginBusiCtrl", ["$scope","commonService","$rootScope",function($scope,commonService,$rootScope) {
	var referrer = '';
	var login={
			isBrowserShow:false,
			companyName:"",//公司名称
			...
			queryCompanyInfo:function(){
				var that=this;
				commonService.postUrl("portalBusiBO.ajax?cmd=queryCompanyInfoBusi",{tenantCode:$scope.tenantCode},function(data){
			...
			submit:function(version){
				...
				var url = "portalBusiBO.ajax?cmd=doLoginBusi";
				commonService.postUrl(url,login.selectId,function(data){
					...
					if(data != null && data != undefined && data != "" && data.result=="1"){
						...
						window.location = "/toMainBusiness.html?ver="+version;
					}else{
...
/**
	 ...
	 var alert=function(text){
		 var appElement = document.querySelector('[ng-controller=loginBusiCtrl]');
