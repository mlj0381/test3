var indexMasterApp = angular.module("indexMasterApp", ['commonApp']);
indexMasterApp.controller("indexMasterCtrl", ["$scope","commonService","$rootScope",function($scope,commonService,$rootScope) {
	var referrer = '';
	var login={
			isBrowserShow:false,
			companyName:"",//公司名称
			...
			queryCompanyInfo:function(){
				var that=this;
				commonService.postUrl("portalBusiBO.ajax?cmd=queryCompanyInfoPullPagCompany",{tenantCode:$scope.tenantCode},function(data){
			...
			submit:function(version){
				...
				var url = "portalBusiBO.ajax?cmd=doLoginPullPagCompany";
				commonService.postUrl(url,login.selectId,function(data){
					...
					if(data != null && data != undefined && data != "" && data.result=="1"){
						...
						window.location = "/toMainMaster.html?ver="+version;
					}else{
...
/**
	 ...
	 var alert=function(text){
		 var appElement = document.querySelector('[ng-controller=indexMasterCtrl]');
