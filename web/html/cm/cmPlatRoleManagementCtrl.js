var cmPlatRoleManagementApp = angular.module("cmPlatRoleManagementApp", ['commonApp']);
cmPlatRoleManagementApp.controller("cmPlatRoleManagementCtrl", ["$scope","commonService","$timeout",function($scope,commonService,$timeout) { 
	var userInfo ={
		init:function(){
			this.bindEvent();
			this.doQuery();
		...
		//查询列表
		doQuery:function(){
			var url = "sysRoleBO.ajax?cmd=queryRoleCm";
			$timeout(function(){
				$scope.page.load({
							url:url,
		...
		addUser:function(){
			commonService.openTab("333331","新增公司角色","/cm/roleGrant.html");
		},
		updateUser:function(){
			...
			var menuType=selectedDate[0].menuType;
			commonService.openTab("role_"+selectedDate[0].roleId,"新增公司角色",encodeURI("/cm/roleGrant.html?roleId="+selectedDate[0].roleId+"&roleName="+roleName+"&tenantId="+tenantId+"&menuType="+menuType));
		},
